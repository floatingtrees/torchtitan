...
if [ -n "$COMM_MODE" ]; then
    ...
    NGPU="${NGPU}" LOCAL_RANK=0 "$PYTHON_EXECUTABLE" -m torchtitan.train --module ${MODULE} --config ${CONFIG} "$@" --comm.mode=${COMM_MODE} --training.steps 1
else
    # Normal training with torchrun
    PYTORCH_ALLOC_CONF="expandable_segments:True" \
    TORCHFT_LIGHTHOUSE=${TORCHFT_LIGHTHOUSE} \
    "$PYTHON_EXECUTABLE" -m torch.distributed.run \
    --nproc_per_node=${NGPU} --rdzv_backend c10d --rdzv_endpoint="localhost:0" \
