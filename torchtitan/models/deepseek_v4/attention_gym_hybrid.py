...
From the TorchTitan checkout, install the validated Attention Gym revision, its
sparse-kernel dependencies, and the state-retention performance patch::

    pip install cuda-python==13.3.1
    pip install nvidia-cutlass-dsl==4.5.2
    pip install flash-attn-4==4.0.0b17
    pip install quack-kernels==0.5.0
    pip install nvidia-cudnn-frontend==1.27.0
    git clone --branch dev/dsv4-ref \\
        https://github.com/floatingtrees/attention-gym.git ../attention-gym-dsv4
    git -C ../attention-gym-dsv4 checkout \\
        d54b0f2883a100202ab0f69d6a1caea52b17012c
    git -C ../attention-gym-dsv4 apply \\
        "$PWD/torchtitan/models/deepseek_v4/attention_gym_state_retention.patch"
    pip install --no-deps -e ../attention-gym-dsv4

The large-debug MXFP8 benchmark can then be run with::

    NGPU=4 MODULE=deepseek_v4 CONFIG=deepseek_large_debug_mxfp8_config \\
        ./run_train.sh --training.steps 10

This configuration measured 1,563.63 TFLOP/s per GPU on four GB200 GPUs. The
patch retains the CuTe forward state needed by backward instead of recomputing
every attention forward.
...
@override(
    "deepseek_v4_attention_gym_hybrid",
